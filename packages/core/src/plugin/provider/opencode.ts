...
import type { IntegrationOAuthMethodRegistration } from "@opencora/plugin/v2/effect/integration"
import { define } from "@opencora/plugin/v2/effect/plugin"
import type { CredentialValue } from "@opencora/sdk/v2/types"
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load OpenCora provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
