...
import type { Hooks } from "@opencora/plugin"
...
export const ProvidersLoginCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      .positional("url", {
        describe: "opencora auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (provider === "opencode") {
      yield* Prompt.log.info("Create an api key at internal authentication")
    }
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: internal documentation",
      )
    }
