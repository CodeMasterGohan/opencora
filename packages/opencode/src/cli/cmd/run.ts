import type { PermissionV1 } from "@opencora/core/v1/permission"
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@opencora/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run opencora with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running opencora server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'opencora')",
      })
      .option("dir", {
        type: "string",
