...
export function retryable(error: Err, provider: string) {
  ...
  if (SessionV1.APIError.isInstance(error)) {
    ...
    if (error.data.responseBody?.includes("FreeUsageLimitError")) {
      return {
        ...
        action: {
          ...
          message: "Subscribe to OpenCora Go for reliable access to the best open-source models, starting at $5/month.",
          label: "subscribe",
          link: GO_UPSELL_URL,
        },
      }
    ...
    if (error.data.responseBody?.includes("GoUsageLimitError")) {
      ...
      const link = `https://opencora.internal/workspace/${workspace}/go`
      return {
