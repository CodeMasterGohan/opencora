...
test("serializes concurrent auth file updates across service instances", async () => {
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      yield* Effect.all(
        [
          first.updateTokens("posthog", { accessToken: "access-token" }, "https://mcp.internal/mcp"),
          second.updateClientInfo("posthog", { clientId: "client-id" }, "https://mcp.internal/mcp"),
        ],
      ...
      expect(entry?.serverUrl).toBe("https://mcp.internal/mcp")
