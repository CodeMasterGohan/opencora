...
import { LLMEvent, LLMResponse } from "@opencora/llm"
import { RequestExecutor } from "@opencora/llm/route"
...
const zenURL = (connection: string) => `https://opencora.internal/proxy/connections/${connection}/v1`
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "config.json", ...scenario.config(model) }),
    ),
  )
