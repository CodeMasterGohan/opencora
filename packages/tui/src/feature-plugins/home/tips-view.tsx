import type { TuiPluginApi } from "@opencora/plugin/tui"
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public opencora.internal link",
  ...
  "Create {highlight}opencora.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/opencora/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.opencora/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.opencora/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.opencora/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.opencora/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent OpenCora from reading sensitive files",
  "Use {highlight}opencora run{/highlight} for non-interactive scripting",
  "Use {highlight}opencora --continue{/highlight} to resume the last session",
  "Use {highlight}opencora run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}opencora serve{/highlight} for headless API access to OpenCora",
  "Use {highlight}opencora run --attach{/highlight} to connect to a running server",
  "Run {highlight}opencora upgrade{/highlight} to update to the latest version",
  "Run {highlight}opencora auth list{/highlight} to see all configured providers",
  "Run {highlight}opencora agent create{/highlight} for guided agent creation",
  "Use {highlight}/opencora{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}opencora github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/opencora fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/ocora{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.opencora/themes/{/highlight} directory",
  ...
  "Run {highlight}opencora debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/opencora{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with OpenCora Zen for curated, tested models",
