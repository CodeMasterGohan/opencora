...
- Do not replace private `packages/opencode` imports with `@opencora/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@opencora/tui`.
...
Status: Completed. The private `@opencora/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@opencora/tui`.
...
- The package has no dependency on `opencode`, `@opencora/cli`, or
  `@opencora/core`.
...
primitives now live in `@opencora/tui`. The legacy host consumes explicit
...
and build inputs through `@opencora/tui/runtime`. Movable app, component,
...
`@opencora/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@opencora/tui`. Configured references resolve through the new
...
`@opencora/tui`. The legacy host injects a narrow plugin host that retains
...
public `@opencora/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@opencora/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@opencora/core`, or either
  executable package.
...
rg '@opencora/core|packages/opencode|packages/cli' packages/tui
